...
const signup = async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: "User successfully created", data: { name, email } });
    ...
    if (!isMatch) {
     ...
     const getListProduect = async (req, res) => {
      try {
        ...
        res.status(200).json({ message: "List retrieved successfully", data: listData });
      } catch (error) {
      ...
      const update = async (req, res) => {
        try {
          ...
          const updatedProduct = await Produect.findByIdAndUpdate(
            id,
            {
              name: req.body.name,
              size: req.body.size,
